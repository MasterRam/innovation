...
export class PageRepository {
    ...
    public findBy(findQuery: any, callback: (success: boolean, response: any) => void) {
    ...
    public Add(findQuery: any, data: Page, callback: (success: boolean, response: any) => void) {
        const document = new PageDocument(data);
        let error: Error | any;
        if ((error = document.validateSync()) === undefined) {
            PageDocument.findOneAndUpdate(findQuery, data, { new: true }, (err, updatedData) => {
                if (err) {
                    callback(false, err);
                }
